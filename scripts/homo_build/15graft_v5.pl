	...
	if ($ARGV[1] ne "0"){
		...
		open INPUT, "$ARGV[0]_graft_conect.pdb" or die "can not open 10!\n" ;
		@cnc_a_list = ();
		@cnc_b_list = ();
		@line_list = ();
		while(chomp($line=<INPUT>)){
		...
		$atom_c_2 = 0;
		@cyx_num_list = ();
		open OUTPUT, ">$ARGV[0]_graft_conect_2.pdb" or die "can not create 10!\n" ;
		for ($line_i = 0; $line_i < @line_list; $line_i++){
			@gezis = split //, $line_list[$line_i];
			$atom_mark = undef;
			for ($gezis_i = 0; $gezis_i <= 5 ; $gezis_i++){
				$atom_mark .= $gezis[$gezis_i];
			}
			if ($atom_mark eq "ATOM  ") {
				...
				for ($cnc_i = 0; $cnc_i < @cnc_a_list; $cnc_i++){
					if (($resi_name eq "CYS") && ($atom_name_clear eq  "SG") && ($atom_num == $cnc_a_list[$cnc_i])) {
						$part_A = undef;
						for ($gezis_i = 11; $gezis_i <= 16 ; $gezis_i++){
							$part_A .= $gezis[$gezis_i];
						}
						$part_B = undef;
						...
						printf OUTPUT "ATOM  %5g$part_A%3s$part_B\n", $atom_c_2, "CYX";
						$cnc_a_list[$cnc_i] = $atom_c_2;
						$cnc_mark = 1;
						push @cyx_num_list, $resi_num;
						last;
					}
					elsif (($resi_name eq "CYS") && ($atom_name_clear eq  "SG") && ($atom_num == $cnc_b_list[$cnc_i])) {
						$part_A = undef;
						for ($gezis_i = 11; $gezis_i <= 16 ; $gezis_i++){
							$part_A .= $gezis[$gezis_i];
						}
						...
						printf OUTPUT "ATOM  %5g$part_A%3s$part_B\n", $atom_c_2, "CYX";
						$cnc_b_list[$cnc_i] = $atom_c_2;
						$cnc_mark = 1;
						push @cyx_num_list, $resi_num;
						last;
					}
					elsif ($atom_num == $cnc_a_list[$cnc_i]) {
						$part_A = undef;
						for ($gezis_i = 11; $gezis_i < @gezis ; $gezis_i++){
							$part_A .= $gezis[$gezis_i];
						}
						printf OUTPUT "ATOM  %5g$part_A\n", $atom_c_2;
						$cnc_a_list[$cnc_i] = $atom_c_2;
						$cnc_mark = 1;
						last;
					}
					elsif ($atom_num == $cnc_b_list[$cnc_i]) {
						$part_A = undef;
						for ($gezis_i = 11; $gezis_i < @gezis ; $gezis_i++){
							$part_A .= $gezis[$gezis_i];
						}
		...
		print OUTPUT "TER\n";
		for ($cnc_i = 0; $cnc_i < @cnc_a_list; $cnc_i++){
			printf OUTPUT "CONECT%5d%5d\n", $cnc_a_list[$cnc_i], $cnc_b_list[$cnc_i];
		}
		close OUTPUT;
		
		system("mv $ARGV[0]_graft_conect_2.pdb $ARGV[0]_graft_conect_2.pdb_bak");
		open INPUT, "$ARGV[0]_graft_conect_2.pdb_bak" or die "can not open 10!\n" ;
		open OUTPUT, ">$ARGV[0]_graft_conect_2.pdb" or die "can not create 10!\n" ;
		while(chomp($line=<INPUT>)){
			@gezis = split //, $line;
			$atom_mark = undef;
			...
			if ($atom_mark eq "ATOM  ") {
				...
				$cyx_mark = 0;
				for ($cyx_i = 0; $cyx_i < @cyx_num_list; $cyx_i++){
					if ($cyx_num_list[$cyx_i] == $resi_num){
						$part_A = undef;
						for ($gezis_i = 0; $gezis_i <= 16 ; $gezis_i++){
							$part_A .= $gezis[$gezis_i];
						}
		...
		open OUTPUT, ">build_min_graft.in" or die "can not create!\n";
		print OUTPUT "source leaprc.gaff
source leaprc.protein.ff19SB
source leaprc.water.tip3p
source leaprc.lipid21
loadamberparams frcmod.ionsjc_tip3p
com = loadpdb $ARGV[0]_graft_conect_2.pdb
saveAmberParm com $ARGV[0]_graft_start.prmtop $ARGV[0]_graft_start.inpcrd
quit\n";
		close OUTPUT;
		system("tleap -s -f build_min_graft.in");
		
		open OUTPUT, ">Min_graft.in" or die "can not create!\n";
 ...
 /\n";
		...
		system("mpirun --use-hwthread-cpus  -np  $ARGV[2] sander.MPI -O -i Min_graft.in -p $ARGV[0]_graft_start.prmtop -c $ARGV[0]_graft_start.inpcrd  -r Min_graft.ncrst  -o Min_graft.out");
		system("ambpdb -p $ARGV[0]_graft_start.prmtop -c Min_graft.ncrst > $ARGV[0]_graft_conect_prep.pdb");

		system("mv $ARGV[0]_graft_conect_prep.pdb $ARGV[0]_graft_conect_prep.pdb_bak");
		open INPUT, "$ARGV[0]_graft_conect_prep.pdb_bak" or die "can not open 10!\n" ;
		open OUTPUT, ">$ARGV[0]_graft_conect_prep.pdb" or die "can not create 10!\n" ;
		while(chomp($line=<INPUT>)){
			@gezis = split //, $line;
			$atom_mark = undef;
			for ($gezis_i = 0; $gezis_i <= 5 ; $gezis_i++){
