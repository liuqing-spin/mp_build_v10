	sub pdb2fasta {
		...
		while(chomp($xlinex=<XINPUTX>)){
			...
			elsif (($atom_mark eq "ATOM  ") && ($gezis[21] ne $crt_chain_id)  &&  ($ter_mark == 0) && ($crt_resi_name ne "XXXXXX") ) {
				print ">$input_pdbs[0]\t$crt_chain_id\t$resi_count\n";
				for ($aa_i = 0; $aa_i < @aa_tsw; $aa_i++){
					print "$aa_tsw[$aa_i]";
				}
			...
			elsif (($atom_mark eq "ATOM  ") && (($resi_num != $crt_resi_num) || ($resi_name ne $crt_resi_name)) && ($ter_mark == 1)){
				$ter_mark = 0;
				$resi_count++;
				$crt_resi_num = $resi_num;
				$crt_resi_name = $resi_name;
			...
			elsif (($atom_mark eq "ATOM  ") && (($resi_num != $crt_resi_num) || ($resi_name ne $crt_resi_name))){
				$resi_count++;
				$crt_resi_num = $resi_num;
				$crt_resi_name = $resi_name;
				$crt_chain_id = $gezis[21];
