	...
	opendir (OPM, "$m_path/databases/opm_pdbs/") or die "can not open opmdir!\n";
	@opm_db = readdir OPM;
	closedir OPM;
	...
	while(chomp($line=<INPUT>)){
		if ($line_c == 0){
			@temp_arr_2 = split /\s+/, $line;
			push @raw_complex_pdbid, lc($temp_arr_2[-1]);
		}
		$line_c++;
	}
	@temp_arr = split /\./, $com_pdb;
	push @raw_complex_pdbid, lc($temp_arr[0]);
	
	$opm_match = 0;
	for ($raw_i = 0 ; $raw_i < @raw_complex_pdbid; $raw_i++){
		for ($pdb_i = 0 ; $pdb_i < @opm_db; $pdb_i++){
			@temp_arr_3 = split /\./, $opm_db[$pdb_i];
			if ($temp_arr_3[0] eq $raw_complex_pdbid[$raw_i]){
				$opm_match = 1;
				$opm_name = $temp_arr_3[0]."_opm";
				print "opm match! opm name: $opm_name!\n";
				system("cp $m_path/databases/opm_pdbs/$opm_db[$pdb_i]  $opm_name.pdb");
				last;
			}
		}
		if ($opm_match == 1){
	...
	system("mv $opm_name.pdb $opm_name.pdb_bak");

	open INPUT, "$opm_name.pdb_bak" or die "can not open!\n";
	open OUTPUT, ">$opm_name.pdb" or die "can not open!\n";
	while(chomp($line=<INPUT>)){
		@gezis = split //, $line;
		$atom_mark = undef;
		...
		if (($atom_mark eq "ATOM  ") || ($atom_mark eq "HETATM") || ($atom_mark eq "TER   ") || ($atom_mark eq "SSBOND")) {
			print OUTPUT "$line\n";
		}
	}
	close INPUT;
	...
	#system("cp $orig_pdb[0].pdb $orig_pdb[0]_2.pdb");
	...
	print OUTPUT "load $orig_pdb[0].pdb, $orig_pdb[0]_2\n";
	if (@lig_list){
		print OUTPUT "set retain_order, 1\n";
		for ($lig_i = 0; $lig_i < @lig_list; $lig_i++){
			print OUTPUT "load $lig_list[$lig_i]\n";
		}
		print OUTPUT  "create lig_cb, $orig_pdb[0]_2";
		for ($lig_i = 0; $lig_i < @lig_list; $lig_i++){
			@lig_name = split /\./, $lig_list[$lig_i];
			print OUTPUT " or $lig_name[0]";
		}
		print OUTPUT "\n";
		print OUTPUT "align lig_cb and chain $tm_ID, $orig_pdb[0] and chain $tm_ID\n";
		for ($lig_i = 0; $lig_i < @lig_list; $lig_i++){
			@lig_name = split /\./, $lig_list[$lig_i];
			print OUTPUT "save $lig_name[0]_align.pdb, lig_cb and chain $lig_id_list[$lig_i]\n";
			print OUTPUT1 "$lig_name[0]\t$lig_id_list[$lig_i]\n";
		}
	}
	...
	if (@pep_list){
		...
		print OUTPUT  "create pep_cb, $orig_pdb[0]_2";
		for ($pep_i = 0; $pep_i < @pep_list; $pep_i++){
			@pep_name = split /\./, $pep_list[$pep_i];
			print OUTPUT " or $pep_name[0]";
		}
		print OUTPUT "\n";
		print OUTPUT "align pep_cb and chain $tm_ID, $orig_pdb[0] and chain $tm_ID\n";
		for ($pep_i = 0; $pep_i < @pep_list; $pep_i++){
			@pep_name = split /\./, $pep_list[$pep_i];
			print OUTPUT "save $pep_name[0]_align.pdb, pep_cb and chain $pep_id_list[$pep_i]\n";
			print OUTPUT1 "$pep_name[0]\t$pep_id_list[$pep_i]\n";
		}
	}
