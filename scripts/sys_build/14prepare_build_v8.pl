	...
	$prep_resi_num_count = 0;
	$prep_crt_resi_num = -99999;
	$prep_crt_chain_id = "XXXXX";
	$ter_mark = 0;
	open INPUT, "complex_prep_hs.pdb" or die "can not open 22!\n";
	while(chomp($line=<INPUT>)){
		@gezis = split //, $line;
		$atom_mark = undef;
		...
		if (($atom_mark eq "TER   ") ||  ($atom_mark =~ "^TER   .*") || ($atom_mark eq "TER") )  {
			$ter_mark = 1;
			next;
		}
		if (($atom_mark eq "ATOM  ") ||  ($atom_mark eq "HETATM"))  {
			if ($ter_mark == 1){
				$prep_crt_resi_num = $resi_num;
				$prep_crt_chain_id = $gezis[21];
				$prep_resi_num_count++;
				$ter_mark = 0;
				next;
			}
			elsif (($resi_num != $prep_crt_resi_num) || ($prep_crt_chain_id ne $gezis[21])){
				$prep_crt_resi_num = $resi_num;
				$prep_crt_chain_id = $gezis[21];
				$prep_resi_num_count++;
			}
	...
	$biy_resi_num_count = 0;
	$biy_crt_resi_num = -99999;
	$biy_crt_chain_id = "XXXXX";
	$ter_mark = 0;
	open INPUT, "bilayer_complex_prep_hs.pdb" or die "can not open 21!\n";
	$domain_c = 0;
	$domain_mark = 0;
	while(chomp($line=<INPUT>)){
		@gezis = split //, $line;
		...
		if (($atom_mark eq "ATOM  ") ||  ($atom_mark eq "HETATM"))  {
			if ($ter_mark == 1){
				$biy_crt_resi_num = $resi_num;
				$biy_crt_chain_id = $gezis[21];
				$biy_resi_num_count++;
				$ter_mark = 0;
			}
			elsif (($resi_num != $biy_crt_resi_num) || ($biy_crt_chain_id ne $gezis[21])){
				$biy_crt_resi_num = $resi_num;
				$biy_crt_chain_id = $gezis[21];
				$biy_resi_num_count++;
			}
		...
		if ($biy_resi_num_count > $prep_resi_num_count){
			last;
		}
	...
	for ($chain_i = 0; $chain_i < @cid_list ; $chain_i++){
		...
		open OUTPUT, ">$output_file" or die "can not open 23!\n";
	...
	open OUTPUT, ">14align_prep_prot.pml" or die "can not create!\n";
	for ($chain_i = 0; $chain_i < @cid_list ; $chain_i++){
	...
	print OUTPUT "quit\n";
	close OUTPUT;
	system("pymol -qc 14align_prep_prot.pml");
	...
	for ($chain_i = 0; $chain_i < @cid_list ; $chain_i++){
		...
		open INPUT, "aligned_prep_$cid_list[$chain_i].pdb" or die "can not open 4!\n";
		open OUTPUT, ">aligned_prep_ss_$cid_list[$chain_i].pdb" or die "can not create!\n";
		@atomre_lines = ();
		while(chomp($line=<INPUT>)){
		...
		$em_seq = undef;
		$md_seq = undef;
		open INPUT, "./chain_$cid_list[$chain_i]/chain_$cid_list[$chain_i]_mm.aln" or die "can not open 5!\n";
		while(chomp($line=<INPUT>)){
		...
		open INPUT, "$input_cnc_file" or die "can not open 6!\n" ;
		$line_c = 0;
		while(chomp($line=<INPUT>)){
			$line_c++;
			if ($line_c > 1){
		...
		open INPUT, "./chain_$cid_list[$chain_i]/ssbond_filter_model_$cid_list[$chain_i].txt" or die "can not open 7!\n";
		$line_c = 0;
		while(chomp($line=<INPUT>)){
			$line_c++;
			if ($line_c > 0){
	...
	@css_list_b = ();
	@css_list_label_b = ();
	open INPUT, "$orig_pdb" or die "can not create!\n";
	while(chomp($line=<INPUT>)){
		@gezis = split //, $line;
		if ($line=~"^SSBOND.*"){
			if ($gezis[15] eq $gezis[29] ) {
				next;
			}
			$cid_match = 0;
			for ($chain_i = 0; $chain_i < @cid_list ; $chain_i++){
				if ($cid_list[$chain_i] eq $gezis[15]){
					$cid_match++;
				}
				if ($cid_list[$chain_i] eq $gezis[29]){
					$cid_match++;
				}
			}
			if ($cid_match<2){
			...
			open INPUT1, "./chain_$gezis[15]/chain_$gezis[15]_raw_vs_em_resi_num.txt" or die "can not open 8!\n";
			$line_c1 = 0;
			while(chomp($line=<INPUT1>)){
				$line_c1++;
				if ($line_c1==1){
			...
			#system("cp aligned_prep_ss_$gezis[15].pdb   aligned_prep_ss_$gezis[15].pdb_bak ");
			open INPUT1, "aligned_prep_ss_$gezis[15].pdb" or die "can not open 9!\n";
			#open OUTPUT1, ">aligned_prep_ss_$gezis[15].pdb" or die "can not open!\n";
			$csys_a_mark = 0;
			while(chomp($linea=<INPUT1>)){
				@gezisa = split //, $linea;
				$atom_marka = undef;
				for ($gezisa_i = 0; $gezisa_i <= 5; $gezisa_i++){
				        $atom_marka .= "$gezisa[$gezisa_i]";
				}
				$resi_numa = undef;
				for ($gezisa_i = 22; $gezisa_i <= 25 ; $gezisa_i++){
					$resi_numa .= $gezisa[$gezisa_i];
				}
				$resi_namea = undef;
				for ($gezisa_i = 17; $gezisa_i <= 19 ; $gezisa_i++){
					$resi_namea .= $gezisa[$gezisa_i];
				}
				$part_A = undef;
				for ($gezisa_i = 1; $gezisa_i <= 16 ; $gezisa_i++){
					$part_A .= $gezisa[$gezisa_i];
				}
				$part_B = undef;
				for ($gezisa_i = 20; $gezisa_i < @gezisa ; $gezisa_i++){
					$part_B .= $gezisa[$gezisa_i];
				}
				if (($atom_marka eq "ATOM  ") ||  ($atom_marka eq "HETATM"))  {
					if (($resi_numa == $cys_a_md) && (($resi_namea eq "CYX") || ($resi_namea eq "CYS"))){
						#printf OUTPUT1 "$part_A%3s$part_B\n", "CYX";
						$csys_a_mark = 1;
					}
				}
				#else{
			...
			open INPUT1, "./chain_$gezis[29]/chain_$gezis[29]_raw_vs_em_resi_num.txt" or die "can not open 10!\n";
			$line_c1 = 0;
			while(chomp($line=<INPUT1>)){
				$line_c1++;
				if ($line_c1==1){
			...
			#system("cp aligned_prep_ss_$gezis[15].pdb   aligned_prep_ss_$gezis[15].pdb_bak ");
			open INPUT1, "aligned_prep_ss_$gezis[29].pdb" or die "can not open 11!\n";
			#open OUTPUT1, ">aligned_prep_ss_$gezis[15].pdb" or die "can not open!\n";
			$csys_b_mark = 0;
			while(chomp($linea=<INPUT1>)){
				@gezisa = split //, $linea;
				...
				if (($atom_marka eq "ATOM  ") ||  ($atom_marka eq "HETATM"))  {
					if (($resi_numa == $cys_b_md) && (($resi_namea eq "CYX") || ($resi_namea eq "CYS"))){
						#printf OUTPUT1 "$part_A%3s$part_B\n", "CYX";
						$csys_b_mark = 1;
					}
				}
		...
		if (($csys_a_mark == 1) && ($csys_b_mark == 1)){
			push  @css_list_a, $cys_a_md;
			push  @css_list_b, $cys_b_md;
			push  @css_list_label_a, $gezis[15];
			push  @css_list_label_b, $gezis[29];
		} 
	}
	close INPUT;

	for ($css_i = 0; $css_i < @css_list_a; $css_i++){
		system("cp aligned_prep_ss_$css_list_label_a[$css_i].pdb   aligned_prep_ss_$css_list_label_a[$css_i].pdb_bak ");
		open INPUT1, "aligned_prep_ss_$css_list_label_a[$css_i].pdb_bak" or die "can not open 12!\n";
		open OUTPUT1, ">aligned_prep_ss_$css_list_label_a[$css_i].pdb" or die "can not create!\n";
		while(chomp($linea=<INPUT1>)){
			@gezisa = split //, $linea;
			$atom_marka = undef;
			for ($gezisa_i = 0; $gezisa_i <= 5; $gezisa_i++){
			...
			if (($atom_marka eq "ATOM  ") ||  ($atom_marka eq "HETATM"))  {
				if (($resi_numa == $css_list_a[$css_i]) && (($resi_namea eq "CYX") || ($resi_namea eq "CYS"))){
					printf OUTPUT1 "$part_A%3s$part_B\n", "CYX";
				}
				else{
					print OUTPUT1 "$linea\n";
		...
		system("cp aligned_prep_ss_$css_list_label_b[$css_i].pdb   aligned_prep_ss_$css_list_label_b[$css_i].pdb_bak ");
		open INPUT1, "aligned_prep_ss_$css_list_label_b[$css_i].pdb_bak" or die "can not open 13!\n";
		open OUTPUT1, ">aligned_prep_ss_$css_list_label_b[$css_i].pdb" or die "can not open!\n";
		while(chomp($linea=<INPUT1>)){
			@gezisa = split //, $linea;
			$atom_marka = undef;
			for ($gezisa_i = 0; $gezisa_i <= 5; $gezisa_i++){
			...
			if (($atom_marka eq "ATOM  ") ||  ($atom_marka eq "HETATM"))  {
				if (($resi_numa == $css_list_b[$css_i]) && (($resi_namea eq "CYX") || ($resi_namea eq "CYS"))){
					printf OUTPUT1 "$part_A%3s$part_B\n", "CYX";
				}
				else{
					print OUTPUT1 "$linea\n";
        ...
        $biy_crt_resi_num = -99999;
        $ter_mark = 0;
	open INPUT, "bilayer_complex_prep_hs.pdb" or die "can not open 14!\n";
		...
		for ($gezis_i = 71; $gezis_i <= 74 ; $gezis_i++){
			$seg_mark .= $gezis[$gezis_i];
                ...
                if (($atom_mark eq "ATOM  ") ||  ($atom_mark eq "HETATM"))  {
                        ...
                        elsif ($resi_num != $biy_crt_resi_num){
                                $biy_crt_resi_num = $resi_num;
                                $biy_resi_num_count++;
                        }
                }

                if (($seg_mark eq "MEMB") || ($biy_resi_num_count > $prep_resi_num_count)){
                        $protein_mark = 0;
                }
	...
	open INPUT, "nonprotein_forbuild.pdb_bak" or die "can not open 15!\n";
	open OUTPUT, ">nonprotein_forbuild.pdb" or die "can not create!\n";
	$crt_resi_name = "YYY";
	$crt_resi_num = 0;
	$crt_chain_id = "XXX";
	while(chomp($line=<INPUT>)){
		...
		for ($gezis_i = 17; $gezis_i <= 19 ; $gezis_i++){
			$resi_name .= $gezis[$gezis_i];
                ...
                if (($biy_resi_num_count > $prep_resi_num_count) && (($atom_mark eq "ATOM  ") ||  ($atom_mark eq "HETATM")) && ($resi_name ne "WAT") ){
	...
	open OUTPUT, ">temp_complex.pdb" or die "can not open 15!\n";
	for ($chain_i = 0; $chain_i < @cid_list ; $chain_i++){
		open INPUT, "aligned_prep_ss_$cid_list[$chain_i].pdb" or die "can not open 15!\n";
		while(chomp($line=<INPUT>)){
			@gezis = split //, $line;
                        $atom_mark = undef;
                        for ($gezis_i = 0; $gezis_i <= 5; $gezis_i++){
	...
	@raw_pdb = split /\./, $orig_pdb;

	@lig_id_list = ();
	for ($lig_i = 0; $lig_i < @lig_list; $lig_i++){
		open INPUT, "$lig_list[$lig_i]" or die "can not open 15!\n";
		while(chomp($line=<INPUT>)){
			@gezis = split //, $line;
                        $atom_mark = undef;
                        for ($gezis_i = 0; $gezis_i <= 5; $gezis_i++){
	...
	@pep_id_list = ();
	for ($pep_i = 0; $pep_i < @pep_list; $pep_i++){
		open INPUT, "$pep_list[$pep_i]" or die "can not open 15!\n";
		while(chomp($line=<INPUT>)){
			@gezis = split //, $line;
                        $atom_mark = undef;
                        for ($gezis_i = 0; $gezis_i <= 5; $gezis_i++){
	...
	open OUTPUT1, ">14align_lig_pep.pml" or die "can not create!\n";
	print OUTPUT1 "load temp_complex.pdb\n";
	print OUTPUT1 "load $raw_pdb[0].pdb, $raw_pdb[0]_2\n";
	if (@lig_list){
		print OUTPUT1 "set retain_order, 1\n";
		for ($lig_i = 0; $lig_i < @lig_list; $lig_i++){
			print OUTPUT1 "load $lig_list[$lig_i]\n";
		}
		print OUTPUT1  "create lig_cb, $raw_pdb[0]_2";
		for ($lig_i = 0; $lig_i < @lig_list; $lig_i++){
			@lig_name = split /\./, $lig_list[$lig_i];
			print OUTPUT1 " or $lig_name[0]";
		}
		print OUTPUT1 "\n";
		print OUTPUT1 "align lig_cb , temp_complex\n";
		for ($lig_i = 0; $lig_i < @lig_list; $lig_i++){
			@lig_name = split /\./, $lig_list[$lig_i];
			print OUTPUT1 "save aligned_$lig_name[0].pdb, lig_cb and chain $lig_id_list[$lig_i]\n";
		}
	}
	if (@pep_list){
		print OUTPUT1 "set retain_order, 1\n";
		for ($pep_i = 0; $pep_i < @pep_list; $pep_i++){
			print OUTPUT1 "load $pep_list[$pep_i]\n";
		}
		print OUTPUT1  "create pep_cb, $raw_pdb[0]_2";
		for ($pep_i = 0; $pep_i < @pep_list; $pep_i++){
			@pep_name = split /\./, $pep_list[$pep_i];
			print OUTPUT1 " or $pep_name[0]";
		}
		print OUTPUT1 "\n";
		print OUTPUT1 "align pep_cb , temp_complex\n";
		for ($pep_i = 0; $pep_i < @pep_list; $pep_i++){
			@pep_name = split /\./, $pep_list[$pep_i];
			print OUTPUT1 "save aligned_$pep_name[0].pdb, pep_cb and chain $pep_id_list[$pep_i]\n";
		}
	}

	print OUTPUT1 "bg white\nhide line\nshow cartoon\nzoom\n";
	print OUTPUT1 "quit\n";
	close OUTPUT1;

	system("pymol -qc 14align_lig_pep.pml");


	for ($lig_i = 0; $lig_i < @lig_pep_pdb_list; $lig_i++){

		open INPUT, "$lig_pep_pdb_list[$lig_i].pdb" or die "can not open 16!\n";
		@resi_num_list = ();
		@resi_name_list = ();
		@atom_num_list = ();
		@atom_name_list = ();
		...
		open INPUT, "aligned_$lig_pep_pdb_list[$lig_i].pdb" or die "can not open 17!\n";
		open OUTPUT, ">aligned_ss_$lig_pep_pdb_list[$lig_i].pdb" or die "can not create!\n";
		@resi_num_list = ();
		@resi_name_list = ();
		@atom_num_list = ();
		@atom_name_list = ();
	...
	opendir (NAA, "$m_path/databases/nonaa/") or die "can not open opmdir!\n";
	@nonaa_list_temp = readdir NAA;
	@crt_naa_list = ();
	for ($naa_i = 0; $naa_i < @nonaa_list_temp; $naa_i++){
		if ($nonaa_list_temp[$naa_i] =~ ".*prepin"){
			@nonaa_name_sw = split /\./, $nonaa_list_temp[$naa_i];
			push @crt_naa_list, $nonaa_name_sw[0];
		}
	}
	closedir NAA;
	...
	for ($lig_i = 0; $lig_i < @lig_name_list; $lig_i++){
		open INPUT, "aligned_ss_$lig_name_list[$lig_i].pdb" or die "can not open 18!\n";
		while(chomp($line=<INPUT>)){
			@gezis = split //, $line;
			$atom_mark = undef;
			for ($gezis_i = 0; $gezis_i <= 5; $gezis_i++){
			...
			if (($atom_mark eq "ATOM  ") || ($atom_mark eq "HETATM"))   {
				...
				if ($match_naa == 0){
					...
					if ($match_nst == 0){
						for ($naa_i = 0; $naa_i < @crt_naa_list; $naa_i++){
							if ($crt_naa_list[$naa_i] eq $resi_name){
	...
	for ($pep_i = 0; $pep_i < @pep_name_list; $pep_i++){
		open INPUT, "aligned_ss_$pep_name_list[$pep_i].pdb" or die "can not open 19!\n";
		while(chomp($line=<INPUT>)){
			@gezis = split //, $line;
			$atom_mark = undef;
			for ($gezis_i = 0; $gezis_i <= 5; $gezis_i++){
...
quit\n";
	...
	open INPUT, "build_leap_temp.log" or die "can not open 20!\n";
	while(chomp($line=<INPUT>)){
...
loadamberparams frcmod.ionsjc_tip3p\n";
	for ($aa_i = 0; $aa_i < @nst_lig; $aa_i++){
		print OUTPUT "loadamberprep $m_path/databases/nonaa/$nst_lig[$aa_i].prepin\n";
		print OUTPUT "loadamberparams $m_path/databases/nonaa/$nst_lig[$aa_i]_gaff.frcmod\n";
		print OUTPUT "loadoff $m_path/databases/nonaa/$nst_lig[$aa_i].lib\n";
	}
	for ($aa_i = 0; $aa_i < @nst_pep; $aa_i++){
		print OUTPUT "loadamberprep $m_path/databases/nonaa/$nst_pep[$aa_i].prepin\n";
		print OUTPUT "loadamberparams $m_path/databases/nonaa/$nst_pep[$aa_i]_ff14SB.frcmod\n";
		print OUTPUT "loadoff $m_path/databases/nonaa/$nst_pep[$aa_i].lib\n";
	}
	print OUTPUT "npn = loadpdb nonprotein_forbuild.pdb\n";
	...
	for ($css_i = 0; $css_i < @css_list_a; $css_i++){
		for ($chain_i = 0; $chain_i < @cid_list ; $chain_i++){
			if ($css_list_label_a[$css_i] eq $cid_list[$chain_i]){
				$pro_a = "PR$chain_i";
			}
			if ($css_list_label_b[$css_i] eq $cid_list[$chain_i]){
				$pro_b = "PR$chain_i";
			}
		}
		print OUTPUT "bond $pro_a.$css_list_a[$css_i].SG  $pro_b.$css_list_b[$css_i].SG\n";
	}
