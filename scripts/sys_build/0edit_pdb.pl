#system("mv $ARGV[0] origin_$ARGV[0]");
	...
	for ($con_i = 0; $con_i < @con_a_list; $con_i++){
		...
		if (($con_a_mark == 1) && ($con_b_mark == 1)) {
			...
			for ($cyx_i = 0; $cyx_i < @cyx_a_list; $cyx_i++){
				...
				elsif (($cyx_a_list[$cyx_i] == $resi_num_b) && ($cyx_b_list[$cyx_i] == $resi_num_a) && ($cyx_a_chain_list[$cyx_i] eq  $chain_id_b) && ($cyx_b_chain_list[$cyx_i] eq  $chain_id_a)){
					$exist_match = 1;
				}
			}
			if ($exist_match == 0){
	...
	for ($line_i = 0; $line_i < @part_A_list; $line_i++){
		if ($part_A_list[$line_i] eq "TER"){
			...
			printf OUTPUT "$part_A_list[$line_i]%4g    $part_B_list[$line_i]\n", $resi_num_c_list[$line_i];
		}
	}
	close OUTPUT;
