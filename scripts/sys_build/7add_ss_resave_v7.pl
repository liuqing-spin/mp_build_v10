	...
	#if ($ARGV[1] == 0){
	#	system("gmx pdb2gmx -f $ARGV[0].pdb -o $ARGV[0]_2.pdb -ff amber99sb-ildn -water tip3p  -ignh  ")
	#}
	...
	for ($lig_i = 0; $lig_i < @lig_pep_list; $lig_i++){
		...
		for ($cnc_i = 0; $cnc_i < @resi_num_cnc_a_list; $cnc_i++){
			...
			if ($match_ss_mark == 0){
				die "lig or pep structure error 1!\n";
			}
		}
		for ($cnc_i = 0; $cnc_i < @resi_num_cnc_b_list; $cnc_i++){
			...
			if ($match_ss_mark == 0){
				die "lig or pep structure error 2!\n";
			}
		}
		...
		for ($line_i = $line_ini; $line_i < @atomre_lines; $line_i++){
			...
			if ($atom_mark eq "ATOM  "){
				...
				push @atom_num_list_2, $atom_num;
				push @atom_name_list_2, $atom_name;
				push @resi_name_list_2, $resi_name;
				push @chain_ID_list_2, $gezis[21];
				push @pos_x_list_2, $pos_x;
				push @pos_y_list_2, $pos_y;
				push @pos_z_list_2, $pos_z;
		...
		for ($cnc_i = 0; $cnc_i < @resi_num_cnc_a_list; $cnc_i++){
			$match_ss_mark = 0;
			for ($atom_i = 0; $atom_i < @atom_num_list_2; $atom_i++){
				if (($resi_num_cnc_a_list[$cnc_i] == $resi_num_list_2[$atom_i]) &&  ($resi_name_cnc_a_list[$cnc_i] eq $resi_name_list_2[$atom_i])  &&  ($atom_name_cnc_a_list[$cnc_i] eq $atom_name_list_2[$atom_i]) && ($lig_pep_chain_id eq $chain_ID_list_2[$atom_i]) ){
					$match_ss_mark = 1;
					push @cnb_a, $atom_num_list_2[$atom_i];
					last;
				}
			}
			if ($match_ss_mark == 0){
				die "lig or pep structure error 3!\n";
			}
		}
		for ($cnc_i = 0; $cnc_i < @resi_num_cnc_b_list; $cnc_i++){
			$match_ss_mark = 0;
			for ($atom_i = 0; $atom_i < @atom_num_list_2; $atom_i++){
				if (($resi_num_cnc_b_list[$cnc_i] == $resi_num_list_2[$atom_i]) &&  ($resi_name_cnc_b_list[$cnc_i] eq $resi_name_list_2[$atom_i])  &&  ($atom_name_cnc_b_list[$cnc_i] eq $atom_name_list_2[$atom_i]) && ($lig_pep_chain_id eq $chain_ID_list_2[$atom_i]) ){
					$match_ss_mark = 1;
					push @cnb_b, $atom_num_list_2[$atom_i];
					last;
				}
			}
			if ($match_ss_mark == 0){
				die "lig or pep structure error 4!\n";
			}
		}
	...
	if ($ARGV[1] ne "0"){
		...
		system("cp $ARGV[0]_ssinfo.pdb $ARGV[0]_ssinfo_prep.pdb ");
		system("propka3 $ARGV[0]_ssinfo_prep.pdb");
