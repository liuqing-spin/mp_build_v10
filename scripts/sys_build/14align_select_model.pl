	...
	if ($ARGV[2] eq "1"){
		open INPUT, "template_order_list.txt" or die "can not open!\n";
		while(chomp($line=<INPUT>)){
			@itmes = split /\s+/, $line;
			if ($items[0] =~ "^ZT.*"){
				@tem_gezi = split //, $items[0];
				$tem_name = undef;
				for ($gezi_i = 0; $gezi_i <= 3; $gezi_i++){
					$tem_name.= $tem_gezi[$gezi_i];
				}
				system("cp $tem_name.pdb $ARGV[1]_model.pdb");
				last;
			}
		}
		close INPUT;
