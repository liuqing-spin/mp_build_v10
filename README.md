...
bash build.sh \  
  ...
  -f_tem [0/1] \                  # Determines if the user-provided template structure is directly grafted onto the target protein. Default is 0(No).
  -d_opt [0/1] \                  # 0 (keep intermediate files) or 1 (remove intermediate files, default)
 # skip repair parameters
  -f_fix [0/1] \                  # ​Controls whether to skip the protein structure repair step. Default is 1(Do not skip).
  -f_opm [0/OPM file]             # Allows using the specified PDB file as an OPM structure for membrane positioning of the target protein. Default is 0(automatically search).
``` 
...
5.	MPBuild produces reusable modeling structures in the format 'aligned_prep_ss_chainID.pdb' (protein component) and 'aligned_ss_ligand_namd.pdb' (ligand component). If the target protein is confirmed to be a complete and correct structure (e.g., generated by MPBuild), the structural repair step can be skipped using the following command:
```bash
bash build.sh -p_com aligned_prep_ss_A.pdb -p_cid A -p_tmm A \
              -m_path your_path/mp_build_v10/ -f_fix 0
```
6. The file passed to the -p_com flag should preferably retain the HEADER, SSBOND, and CONECT records to facilitate faster OPM structure retrieval. Alternatively, users can provide their own OPM structure and run:
```bash
bash build.sh -p_com aligned_prep_ss_A.pdb -p_cid A -p_tmm A \
              -m_path your_path/mp_build_v10/ -f_fix 0 -f_opm your_opm_file
```
7. The fastest build approach is: in the first round, add the '-d_opt 0' option, retain the build_leap.in file, modify the reproducible files as needed, make corresponding adjustments to build_leap.in, and then execute it directly.
