...
case "$1" in
 -m_path) echo "-m_path option is the pathway of mp_builder, with value $2"
	 ...
	 shift 1
   ;;
 -s_path) echo "-s_path option is the pathway of schrodinger, '0' is default, with value $2"
	 s_path=$2
	 shift 1
   ;;
 ...
 -s_cen) echo "-s_cen option is salt concentration, 0.15 is default, with value $2"
	 ...
	 shift 1
   ;;
 -f_tem) echo "-f_tem option determines whether to directly graft the user-provided template onto the region of the target protein's structure to be repaired. The default value is 0, meaning this feature is not used. with value $2"
	 f_tem=$2
	 shift 1
   ;;
 -f_fix) echo "-f_fix option determines whether to repair the target protein structure. The default value is 1, meaning repair is needed. with value $2"
	 f_fix=$2
	 shift 1
   ;;
 -f_opm) echo "-f_opm option specific a user-provided opm pdb file, which will be used to determine the membrane localization of the target protein and will override the automatic search for the opm ID. The default value is 0, meaning automatic search. with value $2"
	 f_opm=$2
	 shift 1
   ;;
-d_opt) echo "-d_opt option determine whether to remove intermediate files, 1 (yes) is default,  with value $2"
	 d_opt=$2
	 shift 1
   ;;
...
#change!
cp $m_path/scripts/sys_build/0edit_pdb.pl ./
perl 0edit_pdb.pl $p_com
sleep 3s

cp $m_path/scripts/sys_build/1extr_seq.pl ./
perl 1extr_seq.pl edit_$p_com
sleep 3s
...
ids_inpdb="$( bash <<EOF
perl 2extr_chainpdb_v3.pl  edit_$p_com  $p_cid 
EOF
)"
...
do
	if [ $f_fix -eq 0 ]; then
		...
		#change!
		cp $m_path/scripts/homo_build/17find_opm.pl ./
		perl 17find_opm.pl ../edit_$p_com  $cin_id $p_tmm $f_opm  $m_path
		sleep 3s

		#change!
		cp $m_path/scripts/homo_build/18make_ssbond_info.pl ./
		perl 18make_ssbond_info.pl ../edit_$p_com $cin_id 
		sleep 3s

		cp chain_"$cin_id".pdb  chain_"$cin_id"_fe_model_align_fix_refine.pdb
		cp chain_"$cin_id"_re.aln  chain_"$cin_id"_mm.aln
		continue
	fi
	...
	perl 6rank_template_v2.pl ../edit_$p_com  $cin_id $p_tmm  $m_path
	sleep 3s
	...
	perl 11make_ssbond_info_v2.pl ../edit_$p_com $cin_id 
	sleep 3s
	...
	perl 14align_select_model.pl ../edit_$p_com  chain_"$cin_id"_fe  $f_tem
	sleep 3s
	...
	perl 16loop_refine_v2.pl  ../edit_$p_com chain_"$cin_id" $cin_id $s_path
	sleep 3s
done
...
perl 4align_to_opm_v4.pl -p edit_$p_com -tm  $p_tmm  -ci $p_cid -lg $c_lig -lp  $c_pep -mp $m_path -fm $f_opm
sleep 3s
...
#change!
cp $m_path/scripts/sys_build/7add_ss_resave_v7.pl ./
perl 7add_ss_resave_v7.pl complex_align_label_renumber $s_path $p_cid
sleep 3s

#change!
cp $m_path/scripts/sys_build/8hstate_option_amber_v3.pl ./
perl 8hstate_option_amber_v3.pl complex_align_label_renumber_ssinfo $s_path
sleep 3s
...
perl 14prepare_build_v8.pl -ci $p_cid -wt $w_inh  -lg $c_lig -lp  $c_pep  -p edit_$p_com -at $m_path
sleep 3s
